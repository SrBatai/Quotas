"""cabin_hd - HD look-dev rebuild of the hunter's cabin (guidelines v2.1, docs/research/05_graficos_arte.md).

    cd winter-survival/prototypes/lookdev/blender && python3 build_cabin_hd.py

Same contract as the game's cabin.glb (ASSET_SPEC_V2 §17, cutaway parts): Floor, WallBack, WallLeft (+ child
WindowsLeft), WallRight, WallFront (+ child WindowsFront), Roof, Chimney, Porch, DoorAnchor, LanternSocket and the
same Col*-convcolonly boxes (+ ColSteps). Authored in the slice convention (porch toward +Y) and turned 180 deg at
creation (lp.new_scene(authored_front="+Y")) exactly like blender/build_cabin.py, so it drops in with rotation 0.

What changed vs cabin.glb (the gap list of the doc, §1.1):
  * lap siding 0.20 m (was 0.30) + cream corner boards, frieze, skirt; windows with 2x3 mullions, deep frame,
    drip cap and sill carrying a rounded snow line; interior wainscot; an open door leaf;
  * roof: battens (standing seams), cream rake/fascia boards, ridge cap; THICK rounded snow slabs (subdivided
    pillows, 0.22 m, cornice drooping over the eave), partial coverage so the dark roof shows, loose lumps;
  * porch: entry cross-gable with an open cream truss (tie beam, king post, struts), posts with knee braces,
    individual deck boards, dense balusters, rail caps with snow lines, snow on the uncovered deck;
  * chimney: stone base + brick stack with courses and a pillowed snow cap;
  * snow drifts piled against the walls (in Floor, never hidden by the cutaway);
  * hard parts chamfered (1.2-2.5 cm, hardened normals), all snow smooth; baked AO in COLOR_0.a.
"""
import math
import os
import random
...
def build_walls():
    ...
    fr = Face('x', 3.0, 1)
    pane_r, hole_r = window(fr, -0.6, 0.6, 1.25, 2.30, hard, snow, 21)
    lap_siding(slab, fr, -2.46, 2.46, [hole_r])
    corner_boards(hard, fr, [(-2.54, -2.36), (2.36, 2.54)])
    base_bands(hard, fr, -2.36, 2.36)
    wainscot(slab, 'x', 3.0 - T, -1, -2.32, 2.32)
    out["WallRight"] = (slab, hard, snow, [("WindowsRight", pane_r)])
...
def build_collision():
    boxes = {
        ...
        "ColPostL": ((-2.35, 4.32, 0.30), (-2.19, 4.48, 2.44)),
        "ColPostR": ((0.39, 4.32, 0.30), (0.55, 4.48, 2.44)),
    }
...
def build():
    H.new_scene(authored_front="+Y")
    rnd = random.Random(7)
    objs = []
    objs.append(assemble("Floor", *build_floor()))
    walls = build_walls()
    for wname, spec in walls.items():
        ...
        for pname, pmb in panes:
            objs.append(H.flat(H.mk(pmb, pname, parent=w)))
    objs.append(assemble("Roof", *build_roof(rnd), bevel_w=0.018))
    objs.append(assemble("Chimney", *build_chimney(), bevel_w=0.02))
    objs.append(assemble("Porch", *build_porch(rnd), bevel_w=0.012))
    lp.add_empty("DoorAnchor", (-0.9, 3.2, 0.30))
    lp.add_empty("LanternSocket", (-1.6, 4.3, 2.35))
    t = H.bake_ao([o for o in objs], distance=1.2, samples=96)
    print("AO bake %.1f s" % t)
    build_collision()
    glb, tris, surfaces, per = H.export_hd("cabin_hd")
    H.write_manifest_entry("cabin_hd", {
        "file": "cabin_hd.glb", "replaces": "assets/models/cabin.glb", "tris": tris, "surfaces": surfaces,
        "parts": per, "pivot": "footprint centre z=0; porch/door toward +Z Godot (place with rotation 0)",
        "anchors": ["DoorAnchor", "LanternSocket"], "collision": "Col*-convcolonly boxes + ColSteps (as cabin.glb)",
        "note": "WallRight now also has a window (WindowsRight, child of WallRight)."})
    return glb
