...
def build_parts(M, p=None):
    ...
    hips.loft([lp.rrect((0, 0.0, 0.815), 0.238, 0.170, 0.07), lp.rrect((0, 0.0, 0.905), 0.252, 0.180, 0.075),
               lp.rrect((0, 0.0, 0.995), 0.236, 0.165, 0.07)], M["jacket"])
    # ---- jacket middle band (Spine) and chest band + shoulder dome (Chest) -----------------------
    band(mb("Spine"), 0.985, 1.205, 0.250, 0.178, M["jacket"])
    ch = mb("Chest")
    ch.loft([lp.rrect((0, 0.0, 1.195), 0.232, 0.162, 0.07), lp.rrect((0, 0.0, 1.300), 0.245, 0.172, 0.075),
             lp.rrect((0, 0.0, 1.395), 0.228, 0.160, 0.07), lp.rrect((0, 0.0, 1.450), 0.185, 0.135, 0.06),
             lp.rrect((0, 0.005, 1.485), 0.120, 0.100, 0.04)], M["jacket"])
    # zip placket down the front of the three bands (navy, as the slice player)
    for bone, z0, z1, yb in (("Hips", 0.83, 0.99, -0.181), ("Spine", 0.995, 1.195, -0.179),
                             ("Chest", 1.205, 1.40, -0.173)):
    ...
    ch.hexa([(0.035, -0.172, 1.13), (0.105, -0.172, 1.14), (0.035, -0.150, 1.13), (0.105, -0.150, 1.14),
             (0.025, -0.188, 1.43), (0.095, -0.188, 1.43), (0.025, -0.150, 1.43), (0.095, -0.150, 1.43)],
            M["scarf"])
    ch.box((0.030, -0.180, 1.095), (0.110, -0.162, 1.135), M["scarf"])                 # fringe
    # ---- neck + head -----------------------------------------------------------------------------
    ...
    for s, side in ((1, "Left"), (-1, "Right")):
        ...
        rig.limb(mb(side + "Shoulder"), (s * 0.085, 0, 1.405), (s * 0.270, 0, 1.405), (0.105, 0.095),
                 (0.098, 0.098), M["jacket"], 0.045)
        um = mb(side + "UpperArm")
