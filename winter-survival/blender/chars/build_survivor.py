"""Skeletal survivor (player) — ASSET_SPEC_V2 §4, §5.1 (M1); HD v2.1 look in milestone G1.

    cd winter-survival/blender && python3 chars/build_survivor.py

Exports assets/models/chars/survivor_{red,blue,green,mustard}.glb (+ sources/survivor_<variant>.blend and the Godot
`.import` next to each .glb, template lib/export.py kind "char"). Same skeleton as M1 (lib.rig.build_armature(),
27 bones: 22 SkeletonProfileHumanoid names + 5 sockets, T-pose, front -Y), so anims/humanoid_loco.glb plays
unchanged. Each file (ASSET_SPEC_V2 §4.4):

    Armature
    ├─ Body                one palette_vcol surface (COLOR_0 RGBA, A = AO); rigid skin, except the parka skirt
    │                      below the hips, which blends Hips -> Left/RightUpperLeg (<= 2 influences, smoothstep
    │                      rings, as §4.3 allows for long coats) so the hem follows the legs
    └─ Outfit_backpack_m   backpack + bedroll + straps, rigid on Chest (the code may hide / show it by name)

G1 look (docs/research/05_graficos_arte.md §1.5/§4, port of the approved look-dev survivor_hd): natural proportions
(head ~1/7.5 of the height), long parka to mid-thigh with fur-trimmed collar and bunched hood, dark trousers, brown
boots with light sock cuffs, dark gloves and beanie, smooth shaded lofts (10-14 sides) with hard edges only at
hems / cuffs (55 deg). Boot soles touch the ground exactly at the rig contact points (heel y +0.10, ball y -0.10,
tip y -0.19, z 0), the gait generator pivots the feet on them.
...
VARIANTS = {
    "red": dict(parka="parka_rust", beanie="beanie"),
    "blue": dict(parka="parka_navy", beanie="sock"),
    "green": dict(parka="parka_green", beanie="hivis_orange"),
    "mustard": dict(parka="parka_mustard", beanie="beanie"),
}
FIXED = dict(dark="pack_dark", pants="pants_dark", fur="fur", skin="skin_hd", beard="beard", eyes="eyes_dark",
             glove="glove", sock="sock", boots="boots_brown", sole="boots", pack="pack", pack_dark="pack_dark",
             strap="strap", roll="mat_roll")
SUBDIR = "chars"
BUDGET = 3500
# the only 2-influence vertices allowed (parka skirt): Hips + one upper leg (verify_chars / rig.check_rigid_skin)
...
def build(variant):
    ...
    for o in (body, pack):
        H.smooth(o, angle=55)
    problems = rig.check_armature(arm) + rig.check_rigid_skin(body, BLEND_PAIRS) + rig.check_rigid_skin(pack)
    tris = H.tris(body) + H.tris(pack)
    if tris > BUDGET:
        problems.append("tris %d > %d" % (tris, BUDGET))
    if problems:
        raise RuntimeError("survivor_%s: %s" % (variant, "; ".join(problems)))
    name = "survivor_" + variant
    # AO reach 0.25 m for characters (doc 05 §4.6), baked in the T-pose on a ground plane
    glb = export.save_and_export(name, SUBDIR, ao=dict(distance=0.25, samples=64, ground=True))
