...
#   [RENDER=forward] tests/run_multi_shot.sh [out.png] [--clients 2] [--port 7797]
set -uo pipefail
cd "$(dirname "$0")/.."
OUT="${1:-/tmp/ventisca_shots/multi.png}"; shift || true
CLIENTS=2; PORT=7797
while [ $# -gt 0 ]; do case "$1" in --clients) CLIENTS="$2"; shift 2;; --port) PORT="$2"; shift 2;; *) shift;; esac; done
mkdir -p "$(dirname "$OUT")"
DIR="${NET_TEST_OUT:-/tmp/ventisca_multi_shot}"; mkdir -p "$DIR"; rm -f "$DIR"/*.log
...
trap cleanup EXIT
timeout 180 godot --headless --path . -s tests/net/net_smoke.gd ++ --server --config "$DIR/server.cfg" --duration 0 > "$DIR/server.log" 2>&1 < /dev/null &
PIDS+=($!)
for i in $(seq 1 120); do grep -q "\[NET\] READY" "$DIR/server.log" 2>/dev/null && break; sleep 0.5; done
NAMES=(Bea Carlos Dana Eva)
for ((i=0; i<CLIENTS; i++)); do
  timeout 150 godot --headless --path . -s tests/net/net_smoke.gd ++ --client "--name=${NAMES[$i]}" --scenario idle --port "$PORT" --duration 90 --clients "$CLIENTS" > "$DIR/client_$i.log" 2>&1 < /dev/null &
  PIDS+=($!)
  sleep 0.7
done
sleep 6
...
xvfb-run -a -s "-screen 0 1280x720x24" godot --path . "${GODOT_ARGS[@]}" --audio-driver Dummy --resolution 1280x720 \
  -s tests/screenshot.gd ++ --preset=multi --out="$OUT" "--port=$PORT" --wait=6 2>&1 | grep -v -E "ALSA lib|pulse|XDG_RUNTIME|libudev|udev" | grep -E "multi:|screenshot"
