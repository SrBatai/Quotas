...
- **Rendimiento** (`tests/perf_walk.gd`, ruta de 2.26 km a 25 m/s). `--cpu` es headless con la ruta visual forzada y es la puerta de los 2 ms/frame (`perf_budgets.json` `perf_walk_cpu`): p99 ≤ 2 ms, p99.9 ≤ 2.5 ms, ≤ 1.5 % de frames por encima, máx. ≤ 8 ms y 0 frames > 33 ms por streaming. Los picos aislados de 4–6 ms caen en fases triviales distintas en cada ejecución, así que son fallos de página de la VM o expropiación, no código. El modo render (xvfb + Compatibility en llvmpipe, `LP_NUM_THREADS=2`) mide suelo y memoria: con GPU por software cada frame dura 250–350 ms y el rasterizador compite con el hilo principal, así que las cifras de frame de ese modo son solo informativas.
