...
Uso: python3 prep_backgrounds.py <raíz_del_repo> [--cache DIR]   (--cache reutiliza <nombre>_fsrbest.png)
"""
import os
import sys
import numpy as np
...
def main():
    ...
    outdir = os.path.join(root, "docs/research/10_hud_ux/mockups/bg")
    os.makedirs(outdir, exist_ok=True)
    for name, rel in SRC.items():
        cached = os.path.join(cache, f"{name}_fsrbest.png") if cache else None
        img = cv2.imread(cached) if cached and os.path.exists(cached) else clean(cv2.imread(os.path.join(root, rel)))
