...
const NODES_PER_STEP := 4
const MM_PER_STEP := 10
...
var _owner_entry: Array[Dictionary] = [{}, {}, {}]
...
func _step_mesh() -> bool:
	...
	terrain_mesh.mesh = ArrayMesh.new()
	terrain_mesh.material_override = Assets.get_terrain_material()
	terrain_mesh.cast_shadow = GeometryInstance3D.SHADOW_CASTING_SETTING_ON if Quality.allows("terrain_shadows") else GeometryInstance3D.SHADOW_CASTING_SETTING_OFF
...
## (Re)uploads the terrain surface from the cached arrays + current AO.
func rebuild_mesh() -> void:
	...
	arrays[Mesh.ARRAY_CUSTOM0] = data.custom
...
static func _shape_for(v: int) -> Shape3D:
	if _shapes.has(v):
		return _shapes[v]
	var col: Array = ScatterCatalog.VARIANTS[v]["col"]
	var sh: Shape3D = null
	match str(col[0]) if not col.is_empty() else "":
		"cyl":
			var c := CylinderShape3D.new()
			c.radius = float(col[1])
			c.height = float(col[2])
			sh = c
		"sphere":
			var s := SphereShape3D.new()
			s.radius = float(col[1])
			sh = s
		"box":
			var b := BoxShape3D.new()
			b.size = Vector3(float(col[1]), float(col[2]), float(col[3]))
			sh = b
	_shapes[v] = sh
	return sh
...
static func _shape_offset(v: int) -> Vector3:
	...
	if col.is_empty():
		return Vector3.ZERO
	return Vector3(0.0, float(col[col.size() - 1]), 0.0)


func _body(bi: int) -> StaticBody3D:
	while _bodies.size() <= bi:
		var b := StaticBody3D.new()
		b.name = ["Scatter", "Blockers", "Stumps"][_bodies.size()]
		b.collision_layer = [65, 64, 1][_bodies.size()]
		b.collision_mask = 0
...
## All scatter shapes at once, on bodies that are not in the tree yet: Jolt then builds each compound once when
## the body enters the space (≈ 0.2 ms for 150 shapes) instead of once per added shape (≈ 7 ms).
func _step_shapes() -> bool:
	for i in data.entries.size():
		...
		if sh == null:
			continue
		var layer := int(ScatterCatalog.VARIANTS[v]["layer"])
...
func _step_multimesh() -> bool:
	...
	for q in range(_mm_cursor, end):
		...
		mmi.name = "mm_%d" % mk
		mmi.multimesh = mm
		mmi.cast_shadow = GeometryInstance3D.SHADOW_CASTING_SETTING_ON if bool(ScatterCatalog.VARIANTS[v]["shadow"]) else GeometryInstance3D.SHADOW_CASTING_SETTING_OFF
		add_child(mmi)
		_mmis[mk] = mmi
		for n in idx.size():
			entry_mm[idx[n]] = mk
...
func _make_node(e: Dictionary) -> Node3D:
	...
	match int(e["node"]):
		...
		_:
			...
			node.add_child(Assets.spawn_model(str(e["model"])))
...
func _make_stump(i: int) -> void:
	var e: Dictionary = data.entries[i]
	if ScatterCatalog.VARIANTS[int(e["v"])]["kind"] == ScatterCatalog.Kind.LOG:
		return
...
func _set_shape_disabled(i: int, disabled: bool) -> void:
	var o := entry_owner[i]
	if o >= 0:
		_bodies[entry_body[i]].shape_owner_set_disabled(o, disabled)
...
func entry_of_hit(body: Object, shape_index: int) -> int:
	var bi := _bodies.find(body)
	if bi < 0:
		return -1
	var owner_id := (body as StaticBody3D).shape_find_owner(shape_index)
	return int(_owner_entry[bi].get(owner_id, -1))
...
func pick_choppable(from: Vector3, dir: Vector3, max_t: float) -> int:
	...
	for w in wid_index:
		...
		if removed[i] == 1:
			continue
		var e: Dictionary = data.entries[i]
		var pick: Array = ScatterCatalog.VARIANTS[int(e["v"])]["pick"]
