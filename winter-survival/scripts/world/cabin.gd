...
func _ready() -> void:
	...
	spills = WindowSpill.attach_to_windows(self, model, 10.0, 9.0)
	for sp in spills:
		if String(sp.name).contains("Front"):
			sp.base_energy *= 0.12  # the porch window lights the porch floor; the snow beyond stays blue (ref night)
...
func _update_lighting() -> void:
	...
	if dark:
		...
		interior_light.light_energy = 0.5 if stove_on else 0.15
	else:
