...
## `inv` is a PlayerState: the authoritative storage on the server, the owner's mirror on the client.
static func is_already_made(recipe: Dictionary, inv: PlayerState) -> bool:
	if not recipe.get("unique", false) or inv == null:
		return false
	if recipe.has("flag"):
		return bool(inv.get(recipe["flag"]))
...
static func has_materials(recipe: Dictionary, inv: PlayerState) -> bool:
	if inv == null:
		return false
	for id in recipe["cost"]:
		if not inv.has(id, int(recipe["cost"][id])):
			return false
	return true


## Returns {ok: bool, text: String} following GDD §10. Runs on the client (mirror) and again on the server.
static func status(recipe: Dictionary, player: Node) -> Dictionary:
	var inv: PlayerState = player.state if player is Player else null
	if is_already_made(recipe, inv):
		return {"ok": false, "text": STATUS_DONE}
	if not has_materials(recipe, inv):
	...
	if recipe.has("result") and not inv.can_add_result(recipe["result"], recipe["cost"]):
		return {"ok": false, "text": STATUS_FULL}
	return {"ok": true, "text": STATUS_READY}


## Server: consumes costs and gives the result (non-placeable recipes). Returns true on success.
static func craft(recipe: Dictionary, player: Node) -> bool:
	var p := player as Player
	if p == null or p.state.inventory == null:
		return false
	var st := status(recipe, player)
	if not st["ok"]:
		p.state.notify(st["text"], 2.0)
	...
	if recipe.has("result"):
		for id in recipe["result"]:
			var left: int = inv.add(id, int(recipe["result"][id]))
			if left > 0:
				p.drop_item(id, left)
	if recipe.has("flag"):
		inv.set_flag(recipe["flag"], true)
	p.state.emit_sim(&"crafted", [recipe["id"]])
	p.state.notify("Fabricado: %s" % recipe["name"], 3.0)
